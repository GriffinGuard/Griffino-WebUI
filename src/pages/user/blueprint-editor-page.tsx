...
import { ApiError, getPortMismatches } from "@/api/errors";
...
export function BlueprintEditorPage({ mode }: { mode: "create" | "edit" }) {
  ...
  const doSave = async (payload: ReturnType<BlueprintFlowEditorHandle["getPayload"]>) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.code === "BLUEPRINT_PORT_MISMATCH") {
        const mismatches = getPortMismatches(error);
        if (mismatches && mismatches.length > 0) {
          editorRef.current?.highlightMismatches?.(mismatches);
          pushToast({
            title: t("blueprints.toast.portMismatch"),
            description: mismatches
              .map((m) => `${m.fromNodeId} → ${m.toNodeId}: ${m.reason}`)
              .join("; "),
            tone: "error",
          });
        } else {
